...
fn main() {
    ...
    if env::var_os("LENSO_UPDATE_CONTRACT_SNAPSHOT").is_some() {
        write_source_snapshot(&snapshot, Path::new("capability.json"))
            .unwrap_or_else(|error| panic!("failed to update search-index snapshot: {error}"));
    } else {
        check_source_snapshot(&snapshot, Path::new("capability.json")).unwrap_or_else(|error| {
            panic!("Search Index Descriptor or Schemas are stale: {error}")
        });
    }
    check_projection(
    ...
    .unwrap_or_else(|error| panic!("Search Index generated Rust projection is stale: {error}"));
}
